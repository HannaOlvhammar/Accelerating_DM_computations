...
// Compute the electronic transition rates
__global__
void get_rates_and_params(bool element_Ge,
                          ...
                          std::size_t n_params,
                          double* rates,
                          std::size_t n_cols_rates,
                          std::size_t n_cols_cvecs,
                          double* cvecs,
                          double* m_chis,
                          double mass_min,
                          double mass_max,
                          ...
                          double* W5)
{
  ...
  if (iD < data_size) {
  //for (int iD = index; iD < data_size; iD += stride) {
      ...
      if (iQ < n_cols_rates  &&  E > Emin) {
        ...
        for (std::size_t iq = 0; iq < vec_size; iq++) {
          ...
          if (vmin < v_esc + v_E) {
            ...
            get_dm_responses(R, q, E, &cvecs[iD*n_cols_cvecs], j_chi, mu, m_chi);
            double R1v0 = R[0];
            double R1v2 = R[1];
            double ReR2 = R[2];
            double ImR2 = R[3];
            ...
            rates[iD*n_cols_rates + iQ] += term1 + term2 + term3 + term4 + term5 + term6;
          }
        }
      }
    }
    ...
    for (std::size_t iQ = 0; iQ < n_cols_rates; iQ++) {
      rates[iD*n_cols_rates + iQ] *= const_and_differentials; 
    }
  }

  /* The function ends here. It returns void, but has altered the rate arrays to
     contain the data. */
}
