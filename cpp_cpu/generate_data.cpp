...
#include <random>     // size_t
#include <iomanip>     // size_t
#include "units.h"     // physical units
#include "rate_calculator.h" // physical units
...
int main()
{
  ...
  double c_min = 1e-10;
  double c_max = 1.;
  // Number of input parameters for calculating the rate (only mass now) 
  ...
  for (std::size_t i = 0; i < data_size; i++) {
    ...
    cvecs[i*n_cols_cvecs+5]  = c_distribution(gen);
    cvecs[i*n_cols_cvecs+19] = c_distribution(gen);
  }
  ...
  // Prepare the matrices for the rates and parameters
  std::vector<double> rates(data_size*n_Q);


  // Calculate the rates
  get_rates_and_params(
                       element_Ge, element_Si,                            // Determines material
                       data_size, n_params, &rates[0],                    // For saving data
                       n_Q, &cvecs[0], &m_chis[0],                        // Input parameters
                       v0s, v_Es, v_escs, j_chi, rho,                     // System units
                       &E_vec[0], &q_vec[0], vec_size, dE, dq,            // Energy and momenta info
                       &W1[0], &ReW2[0], &ImW2[0], &W3[0], &W4[0], &W5[0] // Crystal response functions
                      );
